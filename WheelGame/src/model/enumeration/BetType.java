...
public enum BetType
{
   RED 
   {
	...
	{
		if(player.getBetType() == RED && winSlot.getColor() == (Color.RED))
		{
   ...
   BLACK 
   {
	...
	public void applyWinLoss(Player player, Slot winSlot)
		{
			if(player.getBetType() == BLACK && winSlot.getColor() == (Color.BLACK))
			{
				player.setPoints(player.getPoints() + player.getBet());
			}
			else
   ...
   ZEROS 
   {
      ...
      public void applyWinLoss(Player player, Slot winSlot)
      {
    	  if(player.getBetType() == ZEROS && winSlot.getColor() == (Color.GREEN0) || player.getBetType() == ZEROS && winSlot.getColor() == (Color.GREEN00))
			{
				player.setPoints(player.getPoints() + player.getBet());
			}
			else
